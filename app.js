...
app.post('/send', ensureAuthenticated, function(req, res){
  var msg = req.user.username + ': ' + req.body.message.toString();
  console.log(msg);
  for (var i = 0, len = users.length; i < len; i++) {
    var user = users[i];
    user.inbox.push(msg);
...
app.get('/messg', ensureAuthenticated, function(req, res){
  res.send(req.user.inbox.pop());
});

app.get('/ulist', ensureAuthenticated, function(req, res){
  var ulist = [];
  for (var i = 0, len = users.length; i < len; i++) {
    var user = users[i];
    if (user.online)
